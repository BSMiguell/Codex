...
const cacheableMatch = sw.match(/function isCacheableAsset\(url\)\s*\{([\s\S]*?)\n\}/);
const cacheableBody = cacheableMatch?.[1] ?? "";

const checks = [
  ["versao 1.4.0", /const VERSION = "aetheria-v1\.4\.0"/.test(sw)],
  ...
  ["videos fora do runtime", !/\.(?:mp4|webm|mov)(?:$|[^a-z0-9])/i.test(cacheableBody)],
  [
...
console.log(`OK — Service Worker v1.4.0 validado; ${checks.length} verificacoes passaram.`);
