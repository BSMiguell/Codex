...
try {
  ...
  // ============================================================
  {
    ...
    check(
      "F5 inicial: /index.html sem erro de console",
      errors.length === 0,
    ...
    check(
      "Hard nav → Mapa: console limpo (sem const duplicado)",
      errors.length === 0,
      `errors=${errors.length}`
    );
    errors.length = 0;
  ...
  // ============================================================
  {
    ...
    check(
      "Direção: link no topo (y=5%)  → 'down'",
      directions["_test-top"] === "down",
      `got=${directions["_test-top"]}`
    );
    check(
      "Direção: link na base (y=95%) → 'up'",
      directions["_test-bottom"] === "up",
      `got=${directions["_test-bottom"]}`
    );
    check(
      "Direção: link à esquerda (x=10%) → 'right'",
      directions["_test-left"] === "right",
      `got=${directions["_test-left"]}`
    );
    check(
      "Direção: link à direita (x=90%)  → 'left'",
      directions["_test-right"] === "left",
      `got=${directions["_test-right"]}`
    );
    ...
    const topDir = await page.evaluate(() =>
      document.documentElement.getAttribute("data-transition-direction")
    );
    check(
      "Clique real em #_test-top: html[data-transition-direction] = 'down'",
      topDir === "down",
      `got=${topDir}`
    );
    ...
    const leftDir = await page.evaluate(() =>
      document.documentElement.getAttribute("data-transition-direction")
    );
    check(
      "Clique real em #_test-left: html[data-transition-direction] = 'right'",
      leftDir === "right",
      `got=${leftDir}`
    );
  ...
  // ============================================================
  {
    ...
    const r2 = await page.evaluate(
      async () => (await fetch("/assets/transition-direction.js")).status
    );
