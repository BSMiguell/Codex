...
async function onboardDone(page) {
  await page.evaluate(() => {
    try {
      localStorage.setItem("aetheria.onboarded", JSON.stringify({ version: "1" }));
    } catch {}
  });
}
async function onboardReset(page) {
  await page.evaluate(() => {
    try {
      localStorage.removeItem("aetheria.onboarded");
    } catch {}
  });
}
...
async function main() {
  ...
  try {
    ...
    // 7b. §9.3 — Search semantica por lore (digita "geada" pra mostrar
    //     regioes + batalha com a palavra "Linha de Geada")
    try {
      await page.locator("#paletteInput").fill("geada");
      // espera o lazy load do search-index.json resolver + render
      await page.waitForFunction(
        () => {
          const labels = [...document.querySelectorAll("#paletteList .pal-group-label")].map((el) =>
            el.textContent.trim()
          );
          return labels.includes("Lore (search semantica)");
        },
        { timeout: 5000 }
      );
      await page.waitForTimeout(400);
      await shot(page, "feat-search-semantica.jpg");
      taken.push("feat-search-semantica.jpg");
    } catch (e) {
      fail("search semantica", e);
    }
    await page.keyboard.press("Escape");
    await page.waitForTimeout(300);

    // 8a. Minimap (W5.1 ja existia desde 02/09; §6.1 Q4 — agora
    //     documentado visualmente). Abre o mapa sem filtro e captura
    //     o canto inferior direito com o bussola 2D.
    await page.goto(`${BASE}/Mapa_Aetheria.html`, { waitUntil: "networkidle" });
    await page.waitForFunction(() => window.__MAPA__, { timeout: 5000 });
    await page.waitForTimeout(2500); // espera a cam assentar + minimap renderizar
    await shot(page, "feat-mapa-minimap.jpg");
    taken.push("feat-mapa-minimap.jpg");
