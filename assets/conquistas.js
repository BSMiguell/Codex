...
(function () {
  ...
  window.AETHERIA_CONQUISTAS = {
    ...
    salvar: function (estado) {
      try {
        localStorage.setItem("aetheria_conquistas", JSON.stringify(estado));
      } catch (e) {}
    },
  ...
  function abrirModalConquistas(est, desbloqueadas) {
    ...
    overlay.style.cssText =
      'position:fixed;inset:0;z-index:300;background:radial-gradient(ellipse at 50% 20%, rgba(227,73,27,0.35) 0%, rgba(23,19,16,0.92) 70%, #0a0a12 100%);display:flex;align-items:center;justify-content:center;padding:2rem;font-family:"Fraunces",Georgia,serif;color:#f0ece4;animation:fadeIn 0.5s ease;';
    overlay.innerHTML =
    ...
    card.style.cssText =
      "position:relative;z-index:2;background:rgba(26,23,20,0.82);backdrop-filter:blur(20px);border:1px solid rgba(227,73,27,0.35);border-radius:2rem;padding:2.5rem 2rem;max-width:640px;width:100%;box-shadow:0 40px 100px -30px rgba(227,73,27,0.25),0 0 0 1px rgba(255,255,255,0.06) inset;animation:fadeIn 0.7s ease 0.15s both;";
    var header = document.createElement("div");
    header.innerHTML =
      '<h2 style="font-family:var(--fd);font-size:1.8rem;letter-spacing:-0.03em;margin:0 0 0.3rem;color:#f4f0e8;text-shadow:0 0 20px rgba(227,73,27,0.4);">🏆 Conquistas do Códice</h2><p style="font-size:0.95rem;color:#b8b0a4;margin:0 0 1.5rem;letter-spacing:0.05em;">Progresso de exploração e interação com o códice.</p>';
    card.appendChild(header);
    var body = document.createElement("div");
    var conquistas = [
      {
        id: "explorador_inicial",
        nome: "Primeiros Passos",
        descricao: "Abriu o Aetheria pela primeira vez.",
        icone: "🌱"
      },
      {
        id: "leitor_22",
        nome: "Leitor das 22 Raças",
        descricao: "Visitou as páginas de todas as 22 raças.",
        icone: "📚"
      },
      {
        id: "mapa_explorador",
        nome: "Explorador do Mapa",
        descricao: "Clicou em pelo menos 10 pins no mapa.",
        icone: "🗺️"
      },
      {
        id: "favoritos_10",
        nome: "Colecionador",
        descricao: "Adicionou 10 personagens aos favoritos.",
        icone: "♥"
      },
      {
        id: "busca_semantica",
        nome: "Investigador",
        descricao: "Usou a busca semântica por lore.",
        icone: "🔎"
      }
    ];
    conquistas.forEach(function (c) {
      ...
      item.style.cssText =
        "display:flex;align-items:center;gap:1.2rem;padding:1rem 0;border-bottom:1px solid rgba(255,255,255,0.06);transition:background 0.3s;";
      item.innerHTML =
        '<div style="font-size:2rem;line-height:1;opacity:' +
        (desbloqueado ? "1" : "0.35") +
        ";filter:" +
        (desbloqueado ? "drop-shadow(0 0 8px rgba(227,73,27,0.5))" : "none") +
        ';">' +
        c.icone +
        '</div><div style="flex:1;"><h3 style="font-family:var(--fs);font-size:1.05rem;font-weight:600;margin:0;color:' +
        (desbloqueado ? "#f4f0e8" : "#7a746a") +
        ';letter-spacing:0.02em;">' +
        (desbloqueado ? "✓ " : "") +
        c.nome +
        '</h3><p style="font-size:0.85rem;color:#91887a;margin:0.2rem 0 0;line-height:1.5;">' +
        c.descricao +
        '</p></div><span style="font-size:0.75rem;font-weight:600;color:' +
        (desbloqueado ? "#e3491b" : "#5f574b") +
        ";text-transform:uppercase;letter-spacing:0.08em;background:" +
        (desbloqueado ? "rgba(227,73,27,0.15)" : "rgba(255,255,255,0.05)") +
        ";padding:0.3rem 0.7rem;border-radius:2rem;border:1px solid " +
        (desbloqueado ? "rgba(227,73,27,0.35)" : "rgba(255,255,255,0.1)") +
        ';">' +
        (desbloqueado ? "DESBLOQUEADO" : "PENDENTE") +
        "</span>";
    ...
    footer.style.cssText =
      "margin-top:1.5rem;padding-top:1rem;border-top:1px solid rgba(255,255,255,0.08);display:flex;justify-content:space-between;align-items:center;color:#7a746a;font-size:0.85rem;";
    footer.innerHTML =
      "<span>" +
      desbloqueadas.length +
      ' / 5 conquistas</span><button onclick="document.getElementById(\'modal-conquistas\').remove()" style="background:transparent;border:1px solid rgba(255,255,255,0.15);color:#f0e6d2;padding:0.5rem 1.2rem;border-radius:2rem;font-family:var(--fs);font-size:0.85rem;cursor:pointer;transition:all 0.2s;">Fechar</button>';
