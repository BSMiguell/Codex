...
(function () {
  ...
  const RITUALS = {};
  // §11.6 Q4/2026: isReduced() reativo (re-le toda chamada) — antes era
  // const detecta apenas no load; agora reage a body.no-fx + SO.
  // Hierarquia: body.no-fx (toggle do usuario) > prefers-reduced-motion (SO).
  function isReduced() {
  ...
  RITUALS["04_Onis"] = function (card, modal, color) {
    ...
    stage.style.cssText =
      "position:fixed;inset:0;z-index:9999;background:#0a0206;display:flex;align-items:center;justify-content:center;overflow:hidden;pointer-events:none;";
    var video = document.createElement("video");
    var vidPath =
      window.location.pathname.indexOf("racas/") !== -1
        ? "../assets/videos/onis-transition.mp4"
        : "assets/videos/onis-transition.mp4";
    video.src = vidPath;
    ...
    bg.style.cssText =
      "position:absolute;inset:0;background:radial-gradient(circle at 50% 50%, #3d0f0f 0%, #0a0206 70%);z-index:-1;";
  ...
  RITUALS["05_Demonios"] = function (card, modal, color) {
    ...
    var vidPath =
      window.location.pathname.indexOf("racas/") !== -1
        ? "../assets/videos/demonios-transition.mp4"
        : "assets/videos/demonios-transition.mp4";
    video.src = vidPath;
    video.style.cssText = "width:100vw;height:100vh;object-fit:cover;opacity:0.92;";
    video.autoplay = true;
    ...
    video.playsInline = true;
    video.setAttribute("playsinline", "");
    sessionStorage.setItem("ritual_05_played", "1");
    video.setAttribute("webkit-playsinline", "");
    var bg = document.createElement("div");
    bg.style.cssText =
      "position:absolute;inset:0;background:radial-gradient(circle at 50% 50%, #5a0d18 0%, #0a0206 70%);z-index:-1;";
    stage.appendChild(bg);
    stage.appendChild(video);
    document.body.appendChild(stage);
    video.addEventListener("ended", function () {
